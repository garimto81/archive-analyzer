...
| 상태 | 설명 | 표시 |
|------|------|------|
| ✅ **동기화됨** | archive.db와 pokervod.db 모두 존재 | 양쪽 정보 표시 |
| ⚠️ **중복** | 동일 파일명이 여러 경로에 존재 | "중복" 뱃지 |
| ❌ **미등록** | archive.db에만 존재 (HLS 비호환 등) | Target 열 공란 |
| 🔄 **대기중** | 동기화 예정 | "대기" 뱃지 |
